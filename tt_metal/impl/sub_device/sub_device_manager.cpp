...
SubDeviceManager::SubDeviceManager(
    ContextId context_id, tt::stl::Span<const SubDevice> sub_devices, DeviceAddr local_l1_size, IDevice* device) :
    id_(next_sub_device_manager_id_++),
    context_id_(context_id),
    sub_devices_(sub_devices.begin(), sub_devices.end()),
    device_(device),
    local_l1_size_(tt::align(local_l1_size, MetalContext::instance(context_id_).hal().get_alignment(HalMemType::L1))) {
