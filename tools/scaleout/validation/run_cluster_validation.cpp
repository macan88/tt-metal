...
PhysicalSystemDescriptor generate_physical_system_descriptor(const InputArgs& input_args) {
    ...
    auto physical_system_descriptor = run_physical_system_discovery(
        driver_ref, context.get_distributed_context_ptr(), &context.hal(), context.rtoptions().get_target_device());
