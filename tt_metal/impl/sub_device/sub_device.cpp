...
SubDeviceImpl::SubDeviceImpl(const std::array<CoreRangeSet, NumHalProgrammableCoreTypes>& cores, ContextId context_id) :
    context_id_(context_id), cores_(cores) {
    this->validate();
}

SubDeviceImpl::SubDeviceImpl(tt::stl::Span<const CoreRangeSet> cores, ContextId context_id) : context_id_(context_id) {
    TT_FATAL(cores.size() <= this->cores_.size(), "Too many core types for SubDevice");
    std::copy(cores.begin(), cores.end(), this->cores_.begin());
    this->validate();
}

SubDeviceImpl::SubDeviceImpl(std::array<CoreRangeSet, NumHalProgrammableCoreTypes>&& cores, ContextId context_id) :
