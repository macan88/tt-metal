...
tt::tt_metal::DispatchCoreConfig dispatch_core_config(tt::tt_metal::ContextId context_id) {
    return tt::tt_metal::MetalContext::instance(context_id).get_dispatch_core_manager().get_dispatch_core_config();
}

tt_cxy_pair dispatch_core(uint8_t cq_id, tt::tt_metal::ContextId context_id) {
...
DispatchQueryManager::DispatchQueryManager(uint8_t num_hw_cqs, ContextId context_id) : context_id_(context_id) {
