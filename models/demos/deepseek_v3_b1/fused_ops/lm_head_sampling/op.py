...
def _get_max_page_size_and_num_pages(device, num_tiles, tile_size):
    """
...
class LMHeadSampling:
    ...
    def golden(
        ...
        Returns:
            - If fuse_mtp=False: sampled index tensor [1, 1] (uint32)
            - If fuse_mtp=True: tuple of (sampled_index [1, 1] (uint32), mtp_projection_output [1, output_dim] (float32))
        """
        ...
        # Concatenate along the last dimension: [1, hidden_dim + embedding_dim]
    ...
    def op(
        ...
        if enable_mtp:
            ...
            eh_page_size, eh_num_pages = _get_max_page_size_and_num_pages(
        ...
        embedding_cb = 10  # [MTP] Embedding row from DRAM on sender core (intermediate)
        ...
        e_norm_cb = 13  # [MTP] e_rmsnorm output on sender core (intermediate)
        mcast_eh_src_cb = 15  # [MTP] Concat [h_norm|e_norm] on sender core, second mcast source (intermediate)
        argmax_winner_cb = 3
        argmax_gather_cb = 4
        argmax_indices_cb = 5
        argmax_socket_cb = 6
        ...
        mtp_token_bcast_cb = 20  # [MTP] Token CCL broadcast CB, tensor-backed by intermediate_tensor
        ...
        mtp_concat_ready_semaphore_id = 7
        mtp_h_norm_ready_semaphore_id = 8
        ...
        for row in range(mesh_rows):
            for col in range(mesh_cols):
                ...
                # [MTP] Determine if this device has the token (for multi-device MTP)
                # In single-device mode (skip_ccl), the single device has it.
                # In multi-device mode, only the argmax_final_mesh_coord device has it.
                is_mtp_token_source_device = (
                    1
                    if (
                        enable_mtp
                        and (
                            skip_ccl
                            or (
                                argmax_final_mesh_coord is not None
                                and row == int(argmax_final_mesh_coord[0])
                                and col == int(argmax_final_mesh_coord[1])
                            )
                        )
                    )
                    else 0
                ...
                ncrisc_named_compile_time_args = [
                    ...
                    ("is_mtp_token_source_device", is_mtp_token_source_device),
                    # MTP token CCL broadcast CTArgs
                    ("mtp_bcast_cb0_id", mtp_token_bcast_cb if enable_mtp and not skip_ccl else 0),
                    ("mtp_bcast_num_pages_to_read", 1 if enable_mtp and not skip_ccl else 0),
                    ("mtp_bcast_tensor0_page_size", mtp_token_size_bytes if enable_mtp and not skip_ccl else 0),
                    ("mtp_bcast_num_targets_forward_direction", mtp_bcast_num_targets_forward if enable_mtp else 0),
                    ("mtp_bcast_num_targets_backward_direction", mtp_bcast_num_targets_backward if enable_mtp else 0),
                    ("mtp_bcast_is_sender", mtp_bcast_is_sender if enable_mtp else 0),
                    ("mtp_bcast_core_noc_x", core_noc_x if enable_mtp and not skip_ccl else 0),
                    ("mtp_bcast_core_noc_y", core_noc_y if enable_mtp and not skip_ccl else 0),
                    ("mtp_bcast_is_secondary_sender", mtp_bcast_is_secondary_sender if enable_mtp else 0),
                    ("mtp_bcast_has_secondary_target", mtp_bcast_has_secondary_target if enable_mtp else 0),
                    ("mtp_bcast_start_distance_in_hops_forward", mtp_bcast_start_distance_forward if enable_mtp else 0),
                    ("mtp_bcast_range_hops_forward", mtp_bcast_range_hops_forward if enable_mtp else 0),
                    (
                        "mtp_bcast_start_distance_in_hops_backward",
                        mtp_bcast_start_distance_backward if enable_mtp else 0,
                    ),
                    ("mtp_bcast_range_hops_backward", mtp_bcast_range_hops_backward if enable_mtp else 0),
                    # MTP matmul, embedding, rmsnorm CBs
                    ("matmul_eh_in0", mcast_eh_dst_cb),
                    ("matmul_eh_in1", matmul_eh_cb),
                    ("matmul_eh_out", matmul_out_eh_cb),
                    ("matmul_eh_k_num_tiles", eh_num_tiles_k if enable_mtp else 0),
                    ("matmul_eh_out_w", eh_out_w_per_core if enable_mtp else 0),
                    # EH matmul DRAM streaming CTArgs (NCRISC reader)
                    (
                        "matmul_eh_dram_in1_tensor_addr",
                        eh_proj_tensors_per_device[device_idx].buffer_address() if enable_mtp else 0,
                    ),
                    ("matmul_eh_dram_in1_page_size", eh_page_size if enable_mtp else 0),
                    ("matmul_eh_dram_in1_num_pages", eh_num_pages if enable_mtp else 0),
                    ("matmul_eh_dram_in1_block_size_bytes", eh_in1_block_size_bytes if enable_mtp else 0),
                    ("matmul_eh_subblock_k", eh_subblock_k if enable_mtp else 0),
                    ("matmul_eh_num_subblocks_k", eh_num_subblocks_k if enable_mtp else 0),
                    ("matmul_eh_out_num_tiles", eh_out_num_tiles if enable_mtp else 0),
                    ("eh_matmul_num_loop_iters", 1 if enable_mtp else 0),
                    (
                    ...
                    ("rmsnorm_e_output_cb", e_norm_cb),
                    # MTP semaphores
                    ("mtp_ready_semaphore_id", mtp_ready_semaphore_id),
                    ("mtp_concat_ready_semaphore", mtp_concat_ready_semaphore_id if enable_mtp else 0),
                    ("mcast_eh_data_sender_semaphore", mcast_eh_data_sender_semaphore_id if enable_mtp else 0),
                    ...
                    ("mcast_eh_dst_num_pages", eh_num_tiles_k if enable_mtp else 0),
                    ("rmsnorm_h_num_tiles", rms_num_tiles),
                    ("rmsnorm_e_num_tiles", e_num_tiles if enable_mtp else 0),
                    ("mcast_eh_tile_size_bytes", 2048 if enable_mtp else 0),
                    ("embedding_size_bytes", embedding_dim * element_size if enable_mtp else 0),
                    ("mtp_h_norm_ready_semaphore", mtp_h_norm_ready_semaphore_id if enable_mtp else 0),
                ...
                brisc_named_compile_time_args = [
                    ...
                    ("enable_mtp", 1 if enable_mtp else 0),
                    ("is_mtp_token_source_device", is_mtp_token_source_device),
                    # MTP token CCL broadcast CTArgs for BRISC reader
                    ("mtp_bcast_cb0_id", mtp_token_bcast_cb if enable_mtp and not skip_ccl else 0),
                    ("mtp_bcast_num_pages_to_read", 1 if enable_mtp and not skip_ccl else 0),
                    ("mtp_bcast_is_sender", mtp_bcast_is_sender if enable_mtp else 0),
                    ...
                    ("mcast_eh_dest_noc_end_y", mcast_dest_noc_end.y if enable_mtp else 0),
                    ("mcast_eh_num_cores", num_mcast_cores if enable_mtp else 0),
                    ("mcast_eh_data_sender_semaphore", mcast_eh_data_sender_semaphore_id if enable_mtp else 0),
                ...
                trisc_named_compile_time_args = [
                    ...
                    ("rmsnorm_h_output_cb", rmsnorm_input_cb),
                    ("rmsnorm_h_num_tiles", rms_num_tiles),
                    ("mtp_h_norm_ready_semaphore", mtp_h_norm_ready_semaphore_id if enable_mtp else 0),
                    ("rmsnorm_e_input_cb", embedding_cb),
                    ...
                    ("matmul_eh_subblock_w", eh_subblock_w if enable_mtp else 0),
                    ("matmul_eh_subblock_k", eh_subblock_k if enable_mtp else 0),
                    ("matmul_eh_num_subblocks_k", eh_num_subblocks_k if enable_mtp else 0),
                    ("eh_matmul_num_loop_iters", 1 if enable_mtp else 0),
                    (
                ...
                # CB 1: Mcast destination — intermediate buffer on all device grid cores (including sender so
                # BRISC can pass get_write_ptr(mcast_dst_cb) as mcast_receiver_data_addr; excluding sender would
                # save L1 but requires passing receiver's mcast_dst L1 address as a runtime common arg)
                ...
                # [MTP] CB descriptors (only if enable_mtp)
                mtp_cb_descriptors = []
                if enable_mtp:
                    ...
                    embedding_cb_descriptor = ttnn.CBDescriptor(
                        total_size=e_num_tiles * rms_tile_size,
                        core_ranges=mcast_sender_core_grid,
                        format_descriptors=[embedding_cb_format],
                    )

                    # CB 13: e_rmsnorm output - intermediate on sender core (input to concat)
                    e_rmsnorm_out_tile_descriptor = ttnn.TileDescriptor(rms_interpreted_tile)
                    e_rmsnorm_out_cb_format = ttnn.CBFormatDescriptor(
                        buffer_index=e_norm_cb,
                        data_format=data_format,
                        page_size=rms_tile_size,
                        tile=e_rmsnorm_out_tile_descriptor,
                    )
                    e_rmsnorm_out_cb_descriptor = ttnn.CBDescriptor(
                        total_size=e_num_tiles * rms_tile_size,
                        core_ranges=mcast_sender_core_grid,
                        format_descriptors=[e_rmsnorm_out_cb_format],
                    )
                    ...
                    # Uses (1,32) tile format matching in0_tile so the matmul hardware
                    # correctly pairs activation tiles with (32,32) weight tiles.
                    # Total size is the same raw bytes as the concat: 448 * 64 = 28672.
                    ...
                    # CB 20: mtp_token_bcast - tensor-backed by intermediate_tensor on sender core.
                    # After the input logits mcast, intermediate_tensor's buffer is free.
                    # The argmax_final_core writes the token here, and the CCL broadcast
                    # reads from this CB to distribute the token to all devices.
                    mtp_token_bcast_cb_descriptor = ttnn.cb_descriptor_from_sharded_tensor(
                        mtp_token_bcast_cb, intermediate_tensor_device
                    )
                    # Replace format descriptor: constructor accepts ttnn.DataType; the C++ property
                    # setter expects tt::DataFormat, so we build a new descriptor instead of mutating.
                    mtp_token_bcast_cb_descriptor.format_descriptors = [
                        ttnn.CBFormatDescriptor(
                            buffer_index=mtp_token_bcast_cb,
                            data_format=ttnn.uint32,
                            page_size=mtp_token_size_bytes,
                        )
                    ]
                ...
                if enable_mtp:
                    semaphore_descriptors.extend(
                        [
                            ...
                            ttnn.SemaphoreDescriptor(
                                id=mtp_concat_ready_semaphore_id,
                                core_ranges=mcast_sender_core_grid,
                                initial_value=0,
                            ),
                            ttnn.SemaphoreDescriptor(
                                id=mtp_h_norm_ready_semaphore_id,
                                core_ranges=mcast_sender_core_grid,
                                initial_value=0,
                            ),
                        ]
                ...
                unified_kernel = UnifiedKernelDescriptor(
                    ...
                    trisc_common_runtime_args=[
                        epsilon_packed,
                        scalar_packed,
                        float_to_uint32(1.0 / math.sqrt(float(embedding_dim))) if enable_mtp else 0,
                    ],
