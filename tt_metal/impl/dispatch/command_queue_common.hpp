...
uint32_t get_cq_issue_rd_ptr(
    ChipId chip_id, uint8_t cq_id, uint32_t cq_size, ContextId context_id = SILICON_CONTEXT_ID);

template <bool addr_16B>
uint32_t get_cq_issue_wr_ptr(
    ChipId chip_id, uint8_t cq_id, uint32_t cq_size, ContextId context_id = SILICON_CONTEXT_ID);

// has usage in system_memory_manager.cpp
template <bool addr_16B>
uint32_t get_cq_completion_wr_ptr(
    ChipId chip_id, uint8_t cq_id, uint32_t cq_size, ContextId context_id = SILICON_CONTEXT_ID);

template <bool addr_16B>
uint32_t get_cq_completion_rd_ptr(
    ChipId chip_id, uint8_t cq_id, uint32_t cq_size, ContextId context_id = SILICON_CONTEXT_ID);

uint32_t get_cq_dispatch_progress(ChipId chip_id, uint8_t cq_id, ContextId context_id = SILICON_CONTEXT_ID);
