...
SystemMemoryCQInterface::SystemMemoryCQInterface(
    ContextId context_id, uint16_t channel, uint8_t cq_id, uint32_t cq_size, uint32_t cq_start) :
    ...
    TT_ASSERT(
        this->command_completion_region_size %
                    MetalContext::instance(context_id_).hal().get_alignment(HalMemType::HOST) ==
                0 and
