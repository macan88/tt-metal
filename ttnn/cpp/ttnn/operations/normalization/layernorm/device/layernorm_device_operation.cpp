...
void LayerNormDeviceOperation::validate_on_program_cache_miss(
    ...
    if (operation_attributes.distributed_norm_stage == DistributedLayerNormStage::PRE_ALL_GATHER ||
        ...
        TT_FATAL(
            a.padded_shape()[-2] == tile_height,
            "Only activations with batch size equal to tile height ({}) are supported",
            tile_height);
        if (b.has_value()) {
            TT_FATAL(
                b.value().padded_shape()[-2] == tile_height,
                "Only residual tensors with batch size equal to tile height ({}) are supported",
                tile_height);
        }
    }
