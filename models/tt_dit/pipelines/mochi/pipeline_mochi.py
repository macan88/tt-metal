...
class MochiPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.reload_dit_model = (
            reload_dit_model if reload_dit_model is not None else self.mesh_device.get_num_devices() <= 8
        )  # Only required if VAE is memory-constrained.
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            vae_decode = self._vae_decoder_tracer if traced else self.vae.decode
            video = vae_decode(latents, return_dict=False)[0]
            if profiler:
